...
L, L_tilde, ordering = kolesky.noise_cholesky(points, kernel, 2.0, 1.5, noise)
L = L.toarray()
ordered_points = points[ordering]
true = kernel(ordered_points) + np.eye(len(ordered_points)) * noise
approx = np.linalg.inv(L @ L.T) @ L_tilde @ L_tilde.T @ (noise * np.eye(len(ordered_points)))
print(approx)
